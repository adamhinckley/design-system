...
The Button component supports all 22 Tailwind colors with full dark mode support on all non-destructive variants. The `destructive` variant is always styled red to clearly signal dangerous actions:
...
All colors and sizes automatically support dark mode through Tailwind's `dark:` classes on a parent element:

<Canvas of={ButtonStories.DarkModeColorVariants} sourceState="none" />
<Source
  language="tsx"
